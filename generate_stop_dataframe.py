...
#     print(pops)
    ...
    for n in np.arange(n_trials):
        ...
        if stop_signal_channel == "any":
            ...
            if stop_signal_population not in ['FSI','LIPI']:
        ...
        if n in trials_with_stop_signal:
            ...
            for pop in stop_signal_population:
                #print("pop stop_1",pop)
                stop_populations_df.loc[n,pop] = True
                #print(stop_populations_df.loc[n,pop])
    
    return stop_df, stop_channels_df, stop_amplitude_df, stop_onset_df, stop_duration_df, stop_populations_df, stop_list_trials 
...
def GenStopSchedule(stop_signal_probability, actionchannels, n_trials, popdata, stop_signal_channel, stop_signal_amplitude, stop_signal_onset, stop_signal_duration, stop_signal_present, stop_signal_population):
    ...
    stop_df, stop_channels_df, stop_amplitude_df, stop_onset_df, stop_duration_df, stop_populations_df, stop_list_trials = define_stop(stop_signal_probability, actionchannels, n_trials, popdata, stop_signal_channel, stop_signal_amplitude, stop_signal_onset, stop_signal_duration, stop_signal_present, stop_signal_population)
